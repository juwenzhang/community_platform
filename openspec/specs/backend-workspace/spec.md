...
- `prost` + `tonic` + `tonic-health`
- `axum` + `tower` + `tower-http`
- `async-nats`（NATS 客户端）
- `reqwest`（HTTP 客户端，用于 Consul API 调用）
- `uuid`（事件 ID 生成）
- `sea-orm`（ORM 框架，features: runtime-tokio-rustls, sqlx-postgres, with-uuid, with-chrono, postgres-array）
...
- **WHEN** 在 `services/` 目录运行 `cargo build`
- **THEN** `async-nats`、`reqwest`、`uuid`、`sea-orm`、`sea-orm-migration`、`chrono` 等依赖编译成功
